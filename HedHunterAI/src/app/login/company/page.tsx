...
export default function CompanyLoginPage() {
  ...
  async function doSession(token: string) {
    ...
    if (data.role !== "COMPANY") {
      ...
      throw new Error("This is a job seeker account. Please sign in at the job seeker login page.");
    }
  ...
  const handle = async (e: React.FormEvent) => {
    ...
    try {
      ...
      router.push(redirect ?? "/company/dashboard");
  ...
  return (
    <div className="min-h-screen flex flex-col items-center justify-center px-4" style={{ background: "#ffffff" }}>
      ...
      <div className="w-full max-w-sm rounded-2xl p-8" style={{ background: "#f5f7fa", border: "1px solid rgba(91,141,239,.2)" }}>
        {error && (
      ...
      <div className="mt-6 text-center space-y-2">
        <p className="text-sm" style={{ color: "#64748b" }}>
          No account? <Link href="/signup/company" style={{ color: "#5b8def", textDecoration: "none" }}>Create company account</Link>
        </p>
        <p className="text-xs" style={{ color: "#94a3b8" }}>
          Looking for work?{" "}
          <Link href="/login/job-seeker" style={{ color: "#64748b", textDecoration: "none" }}>Job seeker login →</Link>
        </p>
      </div>
    </div>
  );
