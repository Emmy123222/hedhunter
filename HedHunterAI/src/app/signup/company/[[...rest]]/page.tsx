...
export default function SignUpCompanyPage() {
  ...
  const register = async (token: string) => {
    const res = await fetch("/api/auth/register", {
      ...
      body: JSON.stringify({ token, role: "COMPANY" }),
    });
    if (!res.ok) {
  ...
  return (
    <div className="min-h-screen flex flex-col items-center justify-center px-4" style={{ background: "#ffffff" }}>
      <div className="mb-8 text-center">
        ...
        <Logo height={76} href="/" />
        <p className="mt-3 text-sm" style={{ color: "#64748b" }}>Create your company account</p>
      </div>

      <div className="w-full max-w-sm mb-4 p-3 rounded-xl text-xs text-center"
        style={{ background: "rgba(91,141,239,.06)", border: "1px solid rgba(91,141,239,.2)", color: "#475569", fontFamily: "JetBrains Mono,monospace" }}>
        $100/yr company registration + $50 × open positions per job post.
      </div>
      ...
      <div className="w-full max-w-sm rounded-2xl p-8" style={{ background: "#f5f7fa", border: "1px solid rgba(0,0,0,.07)" }}>
        ...
        <form onSubmit={handle} className="flex flex-col gap-4">
          ...
          <button type="submit" disabled={loading}
            ...
            style={{ background: "#5b8def", color: "#fff", opacity: loading ? 0.6 : 1 }}>
      ...
      <p className="mt-6 text-sm" style={{ color: "#64748b" }}>
        Already have an account? <Link href="/login/company" style={{ color: "#5b8def", textDecoration: "none" }}>Sign in</Link>
      </p>
      <p className="mt-2 text-xs text-center max-w-sm" style={{ color: "#94a3b8" }}>
        Looking for work?{" "}
        <Link href="/signup/job-seeker" style={{ color: "#64748b", textDecoration: "none" }}>Register as a job seeker →</Link>
      </p>
    </div>
  );
}
